...
CREATE TABLE finalized_commits (
    finalized_commit_uuid UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- 최종 확정된 커밋의 고유 식별자 (PK)

    request_id UUID NOT NULL REFERENCES commit_generation_requests(request_id) ON DELETE RESTRICT,
    -- 이 최종 커밋의 원본이 되는 커밋 생성 요청 (commit_generation_requests.request_id 참조)
    -- ON DELETE RESTRICT: 최종 확정된 커밋이 있는 요청은 임의로 삭제할 수 없도록 제한 (감사 추적)

    generated_content_id UUID NOT NULL REFERENCES generated_commit_contents(generated_content_id) ON DELETE RESTRICT,
    -- 사용자가 검토한 LLM 생성 커밋 메시지 초안 (generated_commit_contents.generated_content_id 참조)
    ...
    -- 승인자 및 승인 정보
    finalized_by_user_uuid UUID NOT NULL REFERENCES user_info(uuid) ON DELETE SET NULL,
    -- 이 커밋을 최종 확정한 사용자 (user_info.uuid 참조)
...
COMMENT ON COLUMN finalized_commits.finalized_commit_uuid IS '최종 확정된 커밋의 고유 UUID입니다.';
COMMENT ON COLUMN finalized_commits.request_id IS '이 최종 커밋의 원본 커밋 생성 요청 UUID (commit_generation_requests.request_id 참조)입니다.';
COMMENT ON COLUMN finalized_commits.generated_content_id IS '사용자가 검토한 LLM 생성 커밋 메시지 초안의 UUID (generated_commit_contents.generated_content_id 참조)입니다.';
...
COMMENT ON COLUMN finalized_commits.finalized_by_user_uuid IS '이 커밋을 최종 확정한 사용자의 UUID (user_info.uuid 참조)입니다.';
...
-- 인덱스
CREATE INDEX idx_fc_request_id ON finalized_commits(request_id);
CREATE INDEX idx_fc_generated_content_id ON finalized_commits(generated_content_id);
CREATE INDEX idx_fc_finalized_by_user_uuid ON finalized_commits(finalized_by_user_uuid);
CREATE INDEX idx_fc_finalized_timestamp ON finalized_commits(finalized_timestamp DESC);
CREATE INDEX idx_fc_git_commit_sha ON finalized_commits(git_commit_sha) WHERE git_commit_sha IS NOT NULL;
CREATE INDEX idx_fc_git_push_status ON finalized_commits(git_push_status);

...
-- 파티셔닝: 고려 가능 (request_id 또는 scoping_timestamp 기준으로, 데이터가 매우 많을 경우)
-- MVP 중점사항: 요청 ID 참조, 스코핑 단계 구분, 대상 코드 요소, 사용된 분석 방법, 유사도/관련도 점수, 최종 선택 여부.
-- 스케일업 고려사항: RLS, 파티셔닝, 다양한 스코핑 알고리즘 지원, 단계별 임계값 저장, 스코핑 결과의 시각화 지원 데이터.
-- =====================================================================================

CREATE TABLE scoping_results (
    scoping_result_entry_id UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- 스코핑 결과 각 항목의 고유 식별자 (PK)

    request_id UUID NOT NULL REFERENCES commit_generation_requests(request_id) ON DELETE CASCADE,
    -- 이 스코핑 결과가 속한 커밋 생성 요청 (commit_generation_requests.request_id 참조)

    scoping_run_id UUID NOT NULL,               -- 동일 요청 내에서 여러 번의 스코핑 실행(run)이 있을 경우 이를 그룹화하는 ID
                                                -- (예: 사용자가 파라미터를 변경하여 재실행하거나, A/B 테스트 시)
                                                -- 이 ID를 `commit_generation_requests.scoping_result_id` 에서 참조할 수 있음.

    scoping_stage_name TEXT NOT NULL,           -- 스코핑 단계 이름 (예: 'INITIAL_CANDIDATES', 'STATIC_ANALYSIS_FILTER', 'EMBEDDING_SIMILARITY_RANKING', 'FINAL_CONTEXT_SET')
    ...
    -- 스코핑의 대상이 된 코드 요소 인스턴스 (snapshot_code_element_instances.element_instance_uuid 참조)

    -- 스코핑에 사용된 기준 요소 (선택적, 예: 변경된 메인 함수)
    base_element_instance_uuid UUID REFERENCES snapshot_code_element_instances(element_instance_uuid) ON DELETE SET NULL,
    ...
    CONSTRAINT uq_scoping_result_run_stage_target UNIQUE (scoping_run_id, scoping_stage_name, target_element_instance_uuid, scoping_method)
);

COMMENT ON TABLE scoping_results IS '커밋 생성 요청에 대해 수행된 각 스코핑 단계의 결과(선별된 코드 요소 및 점수 등)를 상세히 기록합니다.';
COMMENT ON COLUMN scoping_results.scoping_result_entry_id IS '스코핑 결과 각 항목의 고유 UUID입니다.';
COMMENT ON COLUMN scoping_results.request_id IS '이 스코핑 결과가 속한 커밋 생성 요청의 UUID입니다.';
COMMENT ON COLUMN scoping_results.scoping_run_id IS '동일 요청 내 여러 스코핑 실행을 그룹화하는 ID입니다. 이 ID를 `commit_generation_requests`에서 참조합니다.';
...
COMMENT ON COLUMN scoping_results.target_element_instance_uuid IS '스코핑 대상이 된 코드 요소 인스턴스의 UUID입니다.';
COMMENT ON COLUMN scoping_results.base_element_instance_uuid IS '스코핑의 기준이 된 코드 요소 인스턴스의 UUID (선택적)입니다.';
...
-- 인덱스
CREATE INDEX idx_sr_request_id ON scoping_results(request_id);
CREATE INDEX idx_sr_scoping_run_id ON scoping_results(scoping_run_id);
CREATE INDEX idx_sr_target_element_instance_uuid ON scoping_results(target_element_instance_uuid);
CREATE INDEX idx_sr_stage_order_rank ON scoping_results(scoping_run_id, scoping_stage_order, rank_within_stage);
CREATE INDEX idx_sr_is_selected_for_next_stage ON scoping_results(is_selected_for_next_stage) WHERE is_selected_for_next_stage = TRUE;
CREATE INDEX idx_sr_scoping_method ON scoping_results(scoping_method);

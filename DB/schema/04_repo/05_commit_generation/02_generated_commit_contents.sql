...
-- MVP 중점사항: 요청 ID 참조, 생성된 커밋 메시지(제목/본문), 사용된 LLM 모델 정보, 생성 시각.
...
CREATE TABLE generated_commit_contents (
    generated_content_id UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- 생성된 커밋 콘텐츠의 고유 식별자 (PK)

    request_id UUID NOT NULL REFERENCES commit_generation_requests(request_id) ON DELETE CASCADE,
    -- 이 커밋 콘텐츠가 생성된 원본 커밋 생성 요청 (commit_generation_requests.request_id 참조)
...
COMMENT ON COLUMN generated_commit_contents.generated_content_id IS '생성된 커밋 콘텐츠의 고유 UUID입니다.';
COMMENT ON COLUMN generated_commit_contents.request_id IS '이 커밋 콘텐츠가 생성된 원본 커밋 생성 요청의 UUID (commit_generation_requests.request_id 참조)입니다.';
...
COMMENT ON COLUMN generated_commit_contents.llm_request_log_id IS '실제 LLM API 호출에 대한 로그 ID (llm_request_log.id 참조)입니다. 토큰 사용량, 비용 등의 상세 정보 추적에 사용됩니다.';
...
-- 인덱스
CREATE INDEX idx_gcc_request_id ON generated_commit_contents(request_id);
CREATE INDEX idx_gcc_llm_model_name ON generated_commit_contents(llm_model_name);
CREATE INDEX idx_gcc_generation_timestamp ON generated_commit_contents(generation_timestamp DESC);
CREATE INDEX idx_gcc_is_edited_by_user ON generated_commit_contents(is_edited_by_user);

-- FK 제약조건은 참조하는 테이블(llm_request_log)이 먼저 생성된 후 ALTER TABLE로 추가하는 것을 권장
-- ALTER TABLE generated_commit_contents ADD CONSTRAINT fk_gcc_llm_request_log FOREIGN KEY (llm_request_log_id) REFERENCES llm_request_log(id) ON DELETE SET NULL;

...
-- 파티셔닝: 고려 가능 (request_id 또는 generation_timestamp 기준으로, 데이터가 매우 많을 경우)
-- MVP 중점사항: 요청 ID 및 원본 코드 요소 참조, 생성된 설명서 텍스트, 사용된 LLM 모델 정보.
-- 스케일업 고려사항: RLS, 파티셔닝, 설명서 버전 관리, 사용자 피드백/수정 내역 추적, 설명서 품질 자동 평가 점수.
-- =====================================================================================

CREATE TABLE generated_technical_descriptions (
    tech_description_id UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- 기술 설명서 레코드의 고유 식별자 (PK)

    request_id UUID NOT NULL REFERENCES commit_generation_requests(request_id) ON DELETE CASCADE,
    -- 이 기술 설명서가 생성된 원본 커밋 생성 요청 (commit_generation_requests.request_id 참조)
    ...
    llm_request_log_id UUID, -- REFERENCES llm_request_log(id) ON DELETE SET NULL, (02_llm_module/02_llm_request_log.sql 참조)
                             -- 실제 LLM API 호출 로그 ID (비용, 토큰 사용량 등 상세 정보 추적용)
...
COMMENT ON COLUMN generated_technical_descriptions.tech_description_id IS '기술 설명서 레코드의 고유 UUID입니다.';
COMMENT ON COLUMN generated_technical_descriptions.request_id IS '이 기술 설명서가 생성된 원본 커밋 생성 요청의 UUID입니다.';
COMMENT ON COLUMN generated_technical_descriptions.element_instance_uuid IS '이 기술 설명서가 설명하는 대상 코드 요소 인스턴스의 UUID입니다.';
...
COMMENT ON COLUMN generated_technical_descriptions.llm_request_log_id IS '실제 LLM API 호출에 대한 로그 ID (llm_request_log.id 참조)입니다.';
...
-- 인덱스
CREATE INDEX idx_gtd_request_id ON generated_technical_descriptions(request_id);
CREATE INDEX idx_gtd_element_instance_uuid ON generated_technical_descriptions(element_instance_uuid);
CREATE INDEX idx_gtd_request_element_instance ON generated_technical_descriptions(request_id, element_instance_uuid);
CREATE INDEX idx_gtd_llm_model_name ON generated_technical_descriptions(llm_model_name);
-- (스케일업 시) 버전 관리를 위한 인덱스
-- CREATE INDEX idx_gtd_element_instance_version ON generated_technical_descriptions(element_instance_uuid, version DESC) WHERE is_current_version = TRUE;


-- FK 제약조건은 참조하는 테이블(llm_request_log)이 먼저 생성된 후 ALTER TABLE로 추가하는 것을 권장
-- ALTER TABLE generated_technical_descriptions ADD CONSTRAINT fk_gtd_llm_request_log FOREIGN KEY (llm_request_log_id) REFERENCES llm_request_log(id) ON DELETE SET NULL;

...
-- 파티셔닝: 고려 가능 (repo_uuid, committed_at_on_platform 기준으로, 데이터가 매우 많아질 경우 - 스케일업 시)
...
CREATE TABLE code_snapshots (
    snapshot_uuid UUID PRIMARY KEY DEFAULT gen_random_uuid(),    -- 코드 스냅샷의 고유 식별자 (PK)
    repo_uuid UUID NOT NULL REFERENCES repo_master(repo_uuid) ON DELETE CASCADE, -- 이 스냅샷이 속한 저장소 (FK)
    ...
    CONSTRAINT uq_code_snapshots_repo_commit_hash UNIQUE (repo_uuid, git_commit_hash) -- 한 저장소 내에서 특정 커밋 해시에 대한 스냅샷은 유일해야 함
);

COMMENT ON TABLE code_snapshots IS '특정 Git 커밋을 기준으로 생성된 저장소의 코드 스냅샷 메타데이터를 저장합니다. 모든 분석과 커밋 생성은 이 스냅샷을 기준으로 이루어집니다.';
COMMENT ON COLUMN code_snapshots.snapshot_uuid IS '코드 스냅샷 레코드의 고유 식별 UUID입니다.';
COMMENT ON COLUMN code_snapshots.repo_uuid IS '이 코드 스냅샷이 속한 저장소의 UUID (repo_master.repo_uuid 참조)입니다.';
...
-- 인덱스
CREATE INDEX idx_code_snapshots_repo_commit_hash ON code_snapshots(repo_uuid, git_commit_hash); -- 특정 저장소의 특정 커밋 스냅샷 조회 (UNIQUE 제약으로 커버되지만 명시적 생성도 가능)
CREATE INDEX idx_code_snapshots_repo_status_created_at ON code_snapshots(repo_uuid, analysis_status, snapshot_created_at DESC); -- 특정 저장소의 분석 상태별 최근 스냅샷 조회
CREATE INDEX idx_code_snapshots_committed_at_on_platform ON code_snapshots(committed_at_on_platform DESC); -- 커밋 시간 기준 정렬/조회
CREATE INDEX idx_code_snapshots_analysis_trigger_event ON code_snapshots(analysis_trigger_event);
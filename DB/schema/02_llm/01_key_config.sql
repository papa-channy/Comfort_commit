...
-- MVP 중점사항: 외부 저장소 키 참조, 핵심 제어 변수(우선순위, fallback 여부), Provider별 Rate Limit 명시.
-- 스케일업 고려사항: RLS 적용, 키 사용량 기반 자동 회전 로직 연동, 운영 통계성 필드 추가(llm_request_log 집계 기반).
-- =====================================================================================

CREATE TABLE llm_key_config (
  id SERIAL PRIMARY KEY,
  provider TEXT NOT NULL,                                 -- LLM 제공자 (예: 'fireworks', 'openai', 'anthropic')
  ...
  is_fallback_candidate BOOLEAN DEFAULT FALSE,            -- 주 키 실패 시 대체(fallback) 후보로 사용될 수 있는지 여부
  ...
  rpm_limit INT,                                          -- 해당 키에 대해 Provider가 공식적으로 명시한 분당 요청 수 한도 (Requests Per Minute)
  tpm_limit INT,                                          -- 해당 키에 대해 Provider가 공식적으로 명시한 분당 토큰 수 한도 (Tokens Per Minute)
  
  -- 운영 및 디버깅을 위한 추가 정보 (MVP 선택적 추가 제안)
  disabled_reason TEXT,                                   -- is_active_overall이 FALSE일 경우, 그 사유 (예: 'Provider quota exceeded', 'Manual deactivation for maintenance')
...
COMMENT ON COLUMN llm_key_config.provider IS 'LLM 서비스를 제공하는 회사의 표준 식별자입니다 (예: fireworks, openai).';
...
COMMENT ON COLUMN llm_key_config.is_fallback_candidate IS '주 사용 키에 문제가 발생했을 때, 이 키를 대체 사용 후보로 고려할지 여부를 나타냅니다.';
COMMENT ON COLUMN llm_key_config.priority IS 'LLM 키 선택 로직에서 사용될 우선순위 값입니다. 숫자가 낮을수록 우선순위가 높습니다.';
COMMENT ON COLUMN llm_key_config.rpm_limit IS 'Provider가 이 키에 대해 공식적으로 제한하는 분당 최대 요청 수입니다. 애플리케이션은 이 값을 참고하여 자체적으로 요청을 조절해야 합니다.';
COMMENT ON COLUMN llm_key_config.tpm_limit IS 'Provider가 이 키에 대해 공식적으로 제한하는 분당 최대 토큰 처리량입니다.';
COMMENT ON COLUMN llm_key_config.disabled_reason IS '키가 비활성화된 경우(is_active_overall = FALSE) 그 사유를 기록합니다.';
COMMENT ON COLUMN llm_key_config.last_error_code IS '이 키를 사용한 마지막 API 호출에서 Provider로부터 반환된 오류 코드입니다 (성공 시 NULL).';
...
CREATE INDEX idx_llm_key_config_selection_logic ON llm_key_config(is_active_overall, is_test_only, priority ASC, last_used_at ASC NULLS FIRST);
CREATE INDEX idx_llm_key_config_last_used_at ON llm_key_config(last_used_at);
...
-- 스케일업 고려사항 주석:
-- COMMENT ON COLUMN llm_key_config.id IS '(스케일업 시: 이 테이블에 대한 변경 이력을 llm_key_config_history 테이블에 기록하는 트리거 추가 고려)';

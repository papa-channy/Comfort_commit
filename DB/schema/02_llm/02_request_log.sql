...
-- 스케일업 고려사항: RLS, 파티션 자동 생성/관리 (pg_partman), Cold Storage로의 데이터 아카이빙, prompt/completion 원문 외부(Loki/S3) 저장 및 참조 ID 관리, OpenSearch 연동 (로그 검색/분석).
-- =====================================================================================

CREATE TABLE llm_request_log (
  id BIGSERIAL,                                           -- 내부 자동 증가 ID (PK는 복합키의 일부)
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 로그 생성 시각 (파티셔닝 키)

  uuid UUID REFERENCES user_info(uuid) ON DELETE SET NULL,-- 요청을 발생시킨 사용자 (탈퇴 시 NULL로 익명화)
  key_id INT REFERENCES llm_key_config(id) ON DELETE SET NULL, -- 사용된 API 키 (llm_key_config.id 참조)

  request_correlation_id UUID DEFAULT gen_random_uuid(),  -- 단일 사용자 요청 내 여러 LLM 호출을 그룹핑하기 위한 ID (예: 기술문서 생성 -> 커밋 메시지 생성)
  ...
  provider TEXT NOT NULL,                                 -- 사용된 LLM 제공자 (llm_key_config.provider와 일치)
  ...
  -- 스케일업: Loki/S3 등 외부 저장소로 이전하고, 여기에는 external_prompt_ref_id, external_completion_ref_id 와 같은 참조 ID만 저장.
  ...
  error_code TEXT,                                        -- 실패 시 오류 코드 (Provider 자체 코드 또는 HTTP 상태 코드)
  duration_ms INT,                                        -- LLM API 호출 및 응답 수신까지 총 소요 시간 (밀리초)
  user_latency_ms INT,                                    -- 사용자 관점에서 해당 기능을 사용하는 데 체감한 총 지연 시간 (LLM 호출 시간 포함)

  PRIMARY KEY (id, created_at)                            -- 파티션 테이블의 PK는 파티션 키를 포함해야 함
)
PARTITION BY RANGE (created_at);

COMMENT ON TABLE llm_request_log IS 'LLM API 호출과 관련된 모든 요청, 응답, 비용, 성능 정보를 기록합니다. 파티셔닝을 통해 대용량 로그를 관리합니다.';
COMMENT ON COLUMN llm_request_log.uuid IS 'LLM 요청을 트리거한 사용자의 UUID입니다. 사용자 탈퇴 시 NULL로 설정됩니다.';
COMMENT ON COLUMN llm_request_log.key_id IS 'LLM 호출에 사용된 API 키의 ID (llm_key_config.id 참조)입니다.';
COMMENT ON COLUMN llm_request_log.request_correlation_id IS '하나의 사용자 액션으로 인해 발생하는 여러 단계의 LLM 호출들을 논리적으로 묶기 위한 UUID입니다.';
...
COMMENT ON COLUMN llm_request_log.error_code IS 'LLM API 호출 실패 시 Provider가 반환한 오류 코드 또는 HTTP 상태 코드입니다.';
...
-- 인덱스
CREATE INDEX idx_llm_request_log_uuid_created_at ON llm_request_log(uuid, created_at DESC) WHERE uuid IS NOT NULL; -- 특정 사용자의 최근 LLM 요청 조회
CREATE INDEX idx_llm_request_log_key_id_created_at ON llm_request_log(key_id, created_at DESC) WHERE key_id IS NOT NULL; -- 특정 API 키로 발생한 최근 요청 조회
CREATE INDEX idx_llm_request_log_model_created_at ON llm_request_log(model, created_at DESC); -- 특정 모델 사용 요청 조회
CREATE INDEX idx_llm_request_log_success_created_at ON llm_request_log(success, created_at DESC); -- 성공/실패 요청 필터링
CREATE INDEX idx_llm_request_log_tag_created_at ON llm_request_log(tag, created_at DESC); -- 특정 태그 요청 조회
CREATE INDEX idx_llm_request_log_correlation_id ON llm_request_log(request_correlation_id) WHERE request_correlation_id IS NOT NULL; -- 연관 요청 그룹 조회
-- created_at은 PK의 일부이므로 개별 인덱스 불필요.

-- 스케일업 고려사항 주석:
-- COMMENT ON TABLE llm_request_log IS '(스케일업 시: 프롬프트/응답 외부 저장 후 external_prompt_ref_id, external_completion_ref_id 컬럼 추가. 캐싱 및 재사용성 향상을 위해 original_request_id, embedding_hash, reuse_flag, vector_similarity_score 등의 컬럼 추가 고려.)';
...
CREATE TABLE user_notification_pref (
  ...
  uuid UUID NOT NULL REFERENCES user_info(uuid) ON DELETE CASCADE, -- 사용자 식별자 (user_info.uuid 참조)
  ...
  -- 예시:
  -- {
  --   "on_ide_close": {"action": "trigger_commit_suggestion", "delay_minutes": 1},
...
COMMENT ON COLUMN user_notification_pref.uuid IS 'user_info 테이블의 사용자 UUID를 참조합니다.';
...
COMMENT ON COLUMN user_notification_pref.task_trigger_preferences IS 'Comfort Commit의 특정 기능이 자동으로 시작되거나 알림이 발송되는 조건을 정의하는 JSONB 객체입니다. 예: {"on_ide_close": {"action": "trigger_commit_suggestion"}}';
...
CREATE INDEX idx_user_notification_pref_uuid ON user_notification_pref(uuid); -- 특정 사용자의 알림 설정 조회
CREATE INDEX idx_user_notification_pref_alert_config_gin ON user_notification_pref USING GIN(alert_configurations); -- JSONB 내부 검색 최적화 (예: 특정 알림 타입을 설정한 사용자 검색)
CREATE INDEX idx_user_notification_pref_task_trigger_gin ON user_notification_pref USING GIN(task_trigger_preferences); -- JSONB 내부 검색 최적화

...
-- MVP 중점사항: 핵심 피드백 메타데이터, 관리용 상태 컬럼, 외부 저장소 참조 ID.
...
CREATE TABLE user_feedback_log (
  ...
  uuid UUID REFERENCES user_info(uuid) ON DELETE SET NULL, -- 피드백 남긴 사용자 (탈퇴 시 피드백은 익명으로 보존)
  ...
  external_content_ref_id TEXT UNIQUE,                    -- 외부 저장소(Loki/S3 등)에 저장된 피드백 원문의 참조 ID (Loki/S3의 객체 ID 또는 고유 경로). NULL일 경우 content_summary에 전문 포함 가정.
...
COMMENT ON COLUMN user_feedback_log.uuid IS '피드백을 제출한 사용자의 UUID입니다. 사용자 탈퇴 시 NULL로 설정되어 피드백은 익명으로 보존됩니다.';
COMMENT ON COLUMN user_feedback_log.feedback_type IS '제출된 피드백의 유형입니다 (예: 버그 신고, 기능 제안). 스케일업 시 ENUM 타입 사용을 고려합니다.';
COMMENT ON COLUMN user_feedback_log.content_summary IS '피드백 원문 내용의 요약본 또는 전문(짧은 경우)입니다. 원문이 길 경우 external_content_ref_id를 통해 외부에서 조회합니다.';
COMMENT ON COLUMN user_feedback_log.external_content_ref_id IS 'Loki/S3 등 외부 저장소에 보관된 실제 피드백 원문 데이터에 대한 고유 참조 ID입니다 (예: 객체 스토리지의 경로 또는 로그 시스템의 ID).';
...
CREATE INDEX idx_user_feedback_log_uuid ON user_feedback_log(uuid) WHERE uuid IS NOT NULL; -- 특정 사용자가 남긴 피드백 조회
CREATE INDEX idx_user_feedback_log_feedback_type ON user_feedback_log(feedback_type); -- 피드백 유형별 조회
CREATE INDEX idx_user_feedback_log_created_at ON user_feedback_log(created_at DESC); -- 최근 피드백 순 조회
CREATE INDEX idx_user_feedback_log_is_resolved_created_at ON user_feedback_log(is_resolved, created_at DESC); -- 미처리/처리 피드백 최근 순 조회 (컬럼명 수정 반영)
CREATE INDEX idx_user_feedback_log_external_ref_id ON user_feedback_log(external_content_ref_id) WHERE external_content_ref_id IS NOT NULL; -- 외부 참조 ID로 검색 (컬럼명 수정 반영)

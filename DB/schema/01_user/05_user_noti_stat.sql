...
-- 개인정보보호: 사용자 탈퇴 시 uuid는 NULL로 설정되어 통계는 익명으로 보존될 수 있도록 설계.
-- =====================================================================================

-- 사용자 알림 통계 테이블
CREATE TABLE user_noti_stat (
  id BIGSERIAL PRIMARY KEY,                                -- 내부 자동 증가 ID
  uuid UUID UNIQUE REFERENCES user_info(uuid) ON DELETE SET NULL, -- 사용자 식별자, 탈퇴 시 NULL로 설정되어 익명화. NULL이 아닐 경우 UNIQUE.
  -- UNIQUE 제약은 uuid가 NULL이 아닐 때만 적용됩니다.
...
COMMENT ON COLUMN user_noti_stat.id IS '이 통계 레코드의 고유 식별자입니다.';
COMMENT ON COLUMN user_noti_stat.uuid IS 'user_info 테이블의 사용자 UUID를 참조합니다. 사용자가 탈퇴하면 이 값은 NULL로 설정되어 통계는 익명으로 보존될 수 있습니다. NULL이 아닌 경우 고유해야 합니다.';
...
CREATE INDEX idx_user_noti_stat_uuid ON user_noti_stat(uuid) WHERE uuid IS NOT NULL; -- 활성 사용자 통계 조회용
-- 채널 목록(channels_used) 내 특정 채널 포함 여부 검색이 필요할 경우 GIN 인덱스 고려 (선택적)
-- 예: CREATE INDEX idx_user_noti_stat_channels_gin ON user_noti_stat USING GIN(channels_used);

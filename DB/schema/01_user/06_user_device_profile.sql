...
CREATE TABLE user_device_profile (
  ...
  -- user_info 테이블의 uuid를 참조하며, 사용자 탈퇴 시 관련 디바이스 프로필도 함께 삭제됩니다.

  device_ids_array TEXT[] DEFAULT ARRAY[]::TEXT[],
  -- ✅ 사용자가 등록한 모든 디바이스의 고유 ID 목록 (배열).
  -- 이 배열은 devices_info JSONB 내의 device_id들과 일관성을 유지해야 하며, 특정 device_id의 등록 여부 빠른 검색에 사용됩니다 (GIN 인덱스).

  devices_info JSONB DEFAULT '[]'::JSONB,
  -- ✅ 각 디바이스의 상세 정보를 담는 JSON 객체 배열.
  -- 예시 내부 객체 구조:
  -- {
  --   "device_id": "client_generated_unique_id_abc123", -- (이 값은 device_ids_array에도 포함됨), 클라이언트에서 생성 및 관리
...
COMMENT ON COLUMN user_device_profile.uuid IS 'user_info 테이블의 사용자 UUID를 참조하는 기본 키입니다.';
COMMENT ON COLUMN user_device_profile.device_ids_array IS '사용자가 등록한 모든 디바이스의 고유 ID를 문자열 배열로 저장합니다. 특정 디바이스 ID 검색에 사용됩니다.';
COMMENT ON COLUMN user_device_profile.devices_info IS '각 디바이스의 상세 정보(이름, 타입, OS, 신뢰/차단 상태, 사용 통계 등)를 JSON 객체 배열 형태로 저장합니다. 각 객체는 "device_id"를 포함해야 합니다.';
...
-- uuid는 PRIMARY KEY이므로 자동으로 인덱싱됩니다.
CREATE INDEX idx_user_device_profile_device_ids_gin ON user_device_profile USING GIN(device_ids_array); -- 특정 device_id가 등록되었는지 빠르게 검색
-- devices_info JSONB 내부 특정 필드 검색이 자주 필요할 경우, 표현식 기반 GIN 인덱스 추가 고려 (스케일업 시)
-- 예: CREATE INDEX idx_user_device_profile_devices_info_device_id_gin ON user_device_profile USING GIN ((devices_info -> 'device_id'));
-- 예: CREATE INDEX idx_user_device_profile_devices_info_trusted_gin ON user_device_profile USING GIN ((devices_info -> 'is_trusted_device'));

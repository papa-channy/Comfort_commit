...
CREATE TABLE user_secret (
  ...
  --     "key_reference_location": "env_var_or_vault_path", -- 예: "env:FIREWORKS_API_KEY_USER_XYZ" 또는 "vault:secret/data/user/uuid/fireworks"
  --     "last_used_successfully_at": "YYYY-MM-DDTHH:MM:SSZ",
  --     "status": "active" -- (ENUM 유사: 'active', 'revoked_by_user', 'auto_revoked_due_to_expiry', 'provider_disabled')
  --   }
  -- }
  ...
  --     "refresh_token_storage_info": "vault:secret/data/user/uuid/github_refresh_token", -- 실제 Refresh Token 저장 위치 참조
  --     "access_token_redis_key_pattern": "user_access_token:uuid:github", -- Access Token이 저장된 Redis 키 (패턴 또는 실제 키)
  --     "access_token_expires_at": "YYYY-MM-DDTHH:MM:SSZ", -- (Redis TTL과 동기화되거나, 여기서 관리)
  --     "last_refreshed_at": "YYYY-MM-DDTHH:MM:SSZ",
  --     "status": "active" -- (ENUM 유사: 'active', 'needs_re_authentication', 'revoked_by_user', 'provider_revoked')
  --   }
  -- }
...
COMMENT ON COLUMN user_secret.uuid IS 'user_info 테이블의 사용자 UUID를 참조하는 기본 키입니다.';
